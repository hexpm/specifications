...
%% Generated by gpb_compile version 3.26.0
-module(hex_pb_names).
...
dfp_read_field_def_Package(<<18, Rest/binary>>, Z1, Z2,
			   F1, F2, TrUserData) ->
    d_field_Package_namespace(Rest, Z1, Z2, F1, F2,
			      TrUserData);
dfp_read_field_def_Package(<<>>, 0, 0, F1, F2, _) ->
    S1 = #{name => F1},
    if F2 == '$undef' -> S1;
       true -> S1#{namespace => F2}
    end;
...
dg_read_field_def_Package(<<0:1, X:7, Rest/binary>>, N,
    ...
    case Key of
      ...
      18 ->
	  d_field_Package_namespace(Rest, 0, 0, F1, F2,
				    TrUserData);
      _ ->
	  case Key band 7 of
	    0 ->
...
merge_msg_Package(#{name := PFname} = PMsg,
		  #{name := NFname} = NMsg, _) ->
    S1 = #{name =>
...
v_msg_Package(#{name := F1} = M, Path, _) ->
    ...
    case M of
      #{namespace := F2} ->
	  v_type_string(F2, [namespace | Path]);
      _ -> ok
    end,
    ok;
