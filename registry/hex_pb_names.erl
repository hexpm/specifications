...
e_msg_Package(#{name := F1}, Bin) ->
    begin
      TrF1 = id(F1), e_type_string(TrF1, <<Bin/binary, 10>>)
    end.

e_msg_Names(Msg) -> e_msg_Names(Msg, <<>>).


e_msg_Names(#{packages := F1}, Bin) ->
    begin
      TrF1 = id(F1),
      if TrF1 == [] -> Bin;
	 true -> e_field_Names_packages(TrF1, Bin)
      end
    end.

e_mfield_Names_packages(Msg, Bin) ->
...
e_field_Names_packages([Elem | Rest], Bin) ->
    Bin2 = <<Bin/binary, 10>>,
    Bin3 = e_mfield_Names_packages(id(Elem), Bin2),
    e_field_Names_packages(Rest, Bin3);
e_field_Names_packages([], Bin) -> Bin.
...
dfp_read_field_def_Package(<<10, Rest/binary>>, Z1, Z2,
			   F1) ->
    d_field_Package_name(Rest, Z1, Z2, F1);
...
dg_read_field_def_Package(<<0:1, X:7, Rest/binary>>, N,
    ...
    Key = X bsl N + Acc,
    case Key of
      10 -> d_field_Package_name(Rest, 0, 0, F1);
      _ ->
	  case Key band 7 of
...
d_field_Package_name(<<0:1, X:7, Rest/binary>>, N, Acc,
    ...
    dfp_read_field_def_Package(Rest2, 0, 0, NewFValue).
...
d_msg_Names(Bin) ->
    dfp_read_field_def_Names(Bin, 0, 0, id([])).

dfp_read_field_def_Names(<<10, Rest/binary>>, Z1, Z2,
			 F1) ->
    d_field_Names_packages(Rest, Z1, Z2, F1);
dfp_read_field_def_Names(<<>>, 0, 0, F1) ->
    #{packages => lists_reverse(F1)};
dfp_read_field_def_Names(Other, Z1, Z2, F1) ->
    dg_read_field_def_Names(Other, Z1, Z2, F1).
...
dg_read_field_def_Names(<<0:1, X:7, Rest/binary>>, N,
    ...
    Key = X bsl N + Acc,
    case Key of
      10 -> d_field_Names_packages(Rest, 0, 0, F1);
      _ ->
	  case Key band 7 of
	    0 -> skip_varint_Names(Rest, 0, 0, F1);
	    1 -> skip_64_Names(Rest, 0, 0, F1);
	    2 -> skip_length_delimited_Names(Rest, 0, 0, F1);
	    5 -> skip_32_Names(Rest, 0, 0, F1)
	  end
    end;
dg_read_field_def_Names(<<>>, 0, 0, F1) ->
    #{packages => lists_reverse(F1)}.
...
skip_varint_Names(<<1:1, _:7, Rest/binary>>, Z1, Z2,
		  F1) ->
    skip_varint_Names(Rest, Z1, Z2, F1);
skip_varint_Names(<<0:1, _:7, Rest/binary>>, Z1, Z2,
		  F1) ->
    dfp_read_field_def_Names(Rest, Z1, Z2, F1).
...
skip_length_delimited_Names(<<0:1, X:7, Rest/binary>>,
    ...
    Length = X bsl N + Acc,
    <<_:Length/binary, Rest2/binary>> = Rest,
    dfp_read_field_def_Names(Rest2, 0, 0, F1).
...
merge_msgs(Prev, New, MsgName) ->
    case MsgName of
      ...
      'Names' -> merge_msg_Names(Prev, New)
    end.
...
verify_msg(Msg, MsgName) ->
    case MsgName of
      ...
      'Names' -> v_msg_Names(Msg, ['Names']);
      _ -> mk_type_error(not_a_known_message, Msg, [])
    end.


-dialyzer({nowarn_function,v_msg_Package/2}).
v_msg_Package(#{name := F1}, Path) ->
...
-dialyzer({nowarn_function,v_msg_Names/2}).
v_msg_Names(#{packages := F1}, Path) ->
    if is_list(F1) ->
    ...
    ok;
v_msg_Names(M, Path) when is_map(M) ->
